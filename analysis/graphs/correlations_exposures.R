...
categories <- list(
  ...
  morbidity = list(vars = vars_morb, label = "Morbidity (Long-Term Conditions)")
)
...
plot_corr_heatmap <- function(
  ...
  p <- ggplot(melted_corr, aes(x = Var1, y = Var2, fill = value)) +
    ...
    theme(
      axis.text.x = element_text(angle = 70, vjust = 1, hjust = 1, size = 9),
...
if (all(c("exp_prop_cons_202209", "exp_prop_cons_mean") %in% names(input))) {
  # Compute per-1,000 patient rates
  input <- input %>%
    mutate(
      cons_sep2022_per1000 = exp_prop_cons_202209 * 1000,
      cons_mean_per1000 = exp_prop_cons_mean * 1000
    )

  # Compute correlation coefficient
  corr_val <- cor(
    input$cons_sep2022_per1000,
  ...
  p_scatter <- ggplot(
    input,
    aes(x = cons_sep2022_per1000, y = cons_mean_per1000)
  ) +
    ...
    labs(
      ...
      y = "Mean consultation rate across months (per 1,000 patients)"
    ) +
    theme(
  ...
  warning(
    "exp_prop_cons_202209 or exp_prop_cons_mean not found in dataset — skipping scatter plot."
  )
}
