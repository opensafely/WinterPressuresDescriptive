...
input <- input %>%
  mutate(
    strata_female_above_p75 = as.integer(sex_female >= cutoffs$sex_female),
    strata_imd1_above_p75 = as.integer(imd_1_most >= cutoffs$imd_1_most),
    strata_eth_white_above_p75 = as.integer(
      ethnicity_white >= cutoffs$ethnicity_white
    ),
    strata_obesity_above_p75 = as.integer(obesity >= cutoffs$obesity),
    strata_smoker_above_p75 = as.integer(
      smoking_current >= cutoffs$smoking_current
    ),
    strata_under5y_above_p75 = as.integer(age_0_4 >= cutoffs$age_0_4),
    strata_age80plus_above_p75 = as.integer(age_80 >= cutoffs$age_80),
    strata_urban_comb_above_p75 = as.integer(
      rurality_urban_comb >= cutoffs$rurality_urban_comb
    )
  )
