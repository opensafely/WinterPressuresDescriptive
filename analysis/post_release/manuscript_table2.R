...
df <- df %>%
  mutate(
    outcome_name = str_remove(outcome_name, paste0("_", group))
  )
...
for (g in groups) {
  ...
  df_table2_wide <- df_table2 %>%
    pivot_wider(
      names_from = cohort,
      values_from = c("n_practices_midpoint6", "Median (IQR)", "Mean (SD)"),
  ...
  results[[g]] <- df_table2_wide
}

df_table2_wide_all <- bind_rows(results)
...
df_table2_wide_all <- df_table2_wide_all %>%
  select(
    subgroup,
    group,
    outcome_label,
    "n_practices_midpoint6 [Pre-COVID]",
    "Median (IQR) [Pre-COVID]",
    "Mean (SD) [Pre-COVID]",
    "n_practices_midpoint6 [Post-COVID 1]",
    "Median (IQR) [Post-COVID 1]",
    "Mean (SD) [Post-COVID 1]",
    "n_practices_midpoint6 [Post-COVID 2]",
    "Median (IQR) [Post-COVID 2]",
    "Mean (SD) [Post-COVID 2]",
    "n_practices_midpoint6 [Post-COVID 3]",
    "Median (IQR) [Post-COVID 3]",
    "Mean (SD) [Post-COVID 3]"
  )

readr::write_csv(df_table2_wide_all, paste0(output_folder, "/table2.csv"), na = "-")
