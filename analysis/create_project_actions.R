...
for (cohort in cohorts_all) {
  ...
  generate_measures_list <- measure_action_names[str_detect(measure_action_names, glue("^generate_measures_{cohort}_"))]
  
  #Actually defining the action to run the data_cleaning.R script for each cohort
  check_and_merge_action <- c(
    comment(glue("Check measures files & generate merged datasets for cohort: {cohort}")),
    action(
